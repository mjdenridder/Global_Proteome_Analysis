...
#define the technical replicates
...
#%% FC plot marker proteins

print("\n\033[4m7. Make fold change plots of the marker proteins\033[0m")

genelist = ['FBP1', 'PCK1', 'ADH2', 'HSP12', 'SNZ1', 'HXT5', 'ANB1', 'COX5A', 'AUS1']
...
for k, gene in enumerate(genelist):
    ...
    for exp in avg_data:
        ...
        if gene in list(avg_data[exp]['Gene name']):
            out      = (pd.DataFrame(avg_data[exp].iloc[np.where(avg_data[exp]['Gene name'].str.contains(pat = gene))[0][0]]).T)
            out_plot = out[[col for col in out.columns if 'Ratio' in col and 'New' not in col]]
            ...
            if out_plot.isnull().sum().sum() < 3:
                ...
                #make a list of amount of detected unique peptides to show in graph
                unique = list(out[[col for col in out.columns if 'Unique' in col]].iloc[0])
                unique = [int(un) for un in unique if un==un]
                
                label  = exp
                lns = axs[i, j].errorbar(xdata, list(out_plot.iloc[0]), fmt = fmt, yerr = yerr, fillstyle = fillstyle[color_i], color = color[color_i], linewidth = 3, markersize = 7, label=exp+ ' ' + str(unique), capsize = 5, elinewidth = 1.2, alpha=0.7, zorder=2)
                leg.append(lns)
    ...
    ax2.set_ylabel('Glucose concentration (mM)', fontsize = 17)
    ...
    axs[i, j].set_title(gene.title(), fontsize = 19, fontweight = 'bold')
    axs[i, j].legend(leg, labs, fontsize = 15, loc='lower right')
    axs[i, j].set_xticks([int(item) for item in xdata_AN]+xdata_O2)
    axs[i, j].set_xticklabels([int(item) for item in xdata_AN]+xdata_O2, fontsize = 15)
    axs[i, j].tick_params(axis='y', labelsize = 15)
    axs[i, j].set_xlabel('Time (h)', fontsize = 17)
    axs[i, j].set_ylabel('Fold change', fontsize = 17)
...
folder  = folder_FC
figname = 'FC_plots_markers.pdf'
fig.savefig(folder + '/' + figname, bbox_inches = 'tight')

#%% make clustermap of average breps
...
#%% 9. calculate p-values AN vs O2

print("\n\033[4m9b. Calculate the p-values for the difference in means between the AN and O2 data\033[0m")

exp_data_WT = pd.DataFrame()
exp_data_MG = pd.DataFrame()
...
for exp in avg_data:
    ...
    if 'WT' in exp:
        keep_cols   = avg_data[exp].columns.difference(exp_data_WT.columns)
        exp_data_WT = pd.concat([exp_data_WT, exp_data[keep_cols]], axis =1)
    elif 'MG' in exp:
        keep_cols   = avg_data[exp].columns.difference(exp_data_WT.columns)
        exp_data_MG = pd.concat([exp_data_MG, exp_data[keep_cols]], axis =1)

for exp in avg_data:
    if 'WT' in exp:
        exp_data = exp_data_WT
    else:
        exp_data = exp_data_MG
    
    #compare the following phases
    xaxis    = ['ME', 'LE', 'ES', 'MS']
    
    for phase in xaxis:
        
        # the ES phase anaerobically should be compared to the ED phase aerobically
        if phase == 'ES':
            
            mean1       = [col for col in exp_data.columns if 'Ratio'  in col and phase in col and 'AN' in col][0]
            mean2       = [col for col in exp_data.columns if 'Ratio'  in col and 'ED' in col and 'O2' in col][0]
            sd1         = [col for col in exp_data.columns if 'sd'  in col and phase in col and 'AN' in col][0]
            sd2         = [col for col in exp_data.columns if 'sd'  in col and 'ED' in col and 'O2' in col][0]
            n1          = [col for col in exp_data.columns if 'num' in col and phase in col and 'AN' in col][0]
            n2          = [col for col in exp_data.columns if 'num' in col and 'ED' in col and 'O2' in col][0]
            
            p_val = st.ttest_ind_from_stats(exp_data[mean1], exp_data[sd1], exp_data[n1], exp_data[mean2], exp_data[sd2], exp_data[n2], equal_var=False, alternative='two-sided')[1]
            p_val = pd.DataFrame(p_val)
            p_val.columns = ['p-value ANvsO2 '+key+'_'+phase]
            p_val.index = exp_data.index
            
            for key in avg_data:
                if 'WT' in exp and 'WT' in key:
                    avg_data[key]['p-value ANvsO2 WT '+phase] = p_val
                elif 'MG' in exp and 'MG' in key:
                    avg_data[key]['p-value ANvsO2 MG '+phase] = p_val
                    
        else:
             
            mean1       = [col for col in exp_data.columns if 'Ratio'  in col and phase in col and 'AN' in col][0]
            mean2       = [col for col in exp_data.columns if 'Ratio'  in col and phase in col and 'O2' in col][0]
            sd1         = [col for col in exp_data.columns if 'sd'  in col and phase in col and 'AN' in col][0]
            sd2         = [col for col in exp_data.columns if 'sd'  in col and phase in col and 'O2' in col][0]
            n1          = [col for col in exp_data.columns if 'num' in col and phase in col and 'AN' in col][0]
            n2          = [col for col in exp_data.columns if 'num' in col and phase in col and 'O2' in col][0]
            
            p_val = st.ttest_ind_from_stats(exp_data[mean1], exp_data[sd1], exp_data[n1], exp_data[mean2], exp_data[sd2], exp_data[n2], equal_var=False, alternative='two-sided')[1]
            p_val = pd.DataFrame(p_val)
            p_val.columns = ['p-value ANvsO2 '+exp+'_'+phase]
            p_val.index = exp_data.index
...
#%% 9. p-values transition into stationary WT

print("\n\033[4m9c. Calculate the p-values for the transition into stationary phase: compare each phase with the previous phase\033[0m")
...
for exp in avg_data:
    ...
    if 'O2' in exp:
        ...
        for i, phase in enumerate(xaxis[:4]):
        
            mean1       = [col for col in exp_data.columns if 'Ratio' in col and xaxis[i] in col ][0]
            mean2       = [col for col in exp_data.columns if 'Ratio' in col and xaxis[i+1] in col][0]
            sd1         = [col for col in exp_data.columns if 'sd' in col and xaxis[i] in col ][0]
            sd2         = [col for col in exp_data.columns if 'sd' in col and xaxis[i+1] in col ][0]
            n1          = [col for col in exp_data.columns if 'num' in col and xaxis[i] in col ][0]
            n2          = [col for col in exp_data.columns if 'num' in col and xaxis[i+1] in col ][0]
            
            p_val = st.ttest_ind_from_stats(exp_data[mean1], exp_data[sd1], exp_data[n1], exp_data[mean2], exp_data[sd2], exp_data[n2], equal_var=False, alternative='two-sided')[1]
            p_val = pd.DataFrame(p_val)
            p_val.columns = ['p-value O2 transition '+exp+'_'+xaxis[i+1]]
            p_val.index = exp_data.index
            
            avg_data[exp]['p-value O2 transition '+exp+'_'+xaxis[i+1]] = p_val
            
            if phase == 'ME':
            
                mean1       = [col for col in exp_data.columns if 'Ratio' in col and xaxis[i] in col ][0]
                mean2       = [col for col in exp_data.columns if 'Ratio' in col and 'MS' in col][0]
                sd1         = [col for col in exp_data.columns if 'sd' in col and xaxis[i] in col ][0]
                sd2         = [col for col in exp_data.columns if 'sd' in col and 'MS' in col ][0]
                n1          = [col for col in exp_data.columns if 'num' in col and xaxis[i] in col ][0]
                n2          = [col for col in exp_data.columns if 'num' in col and 'MS' in col ][0]
                
                p_val = st.ttest_ind_from_stats(exp_data[mean1], exp_data[sd1], exp_data[n1], exp_data[mean2], exp_data[sd2], exp_data[n2], equal_var=False, alternative='two-sided')[1]
                p_val = pd.DataFrame(p_val)
                p_val.columns = ['p-value O2 transition full ' + exp]
                p_val.index = exp_data.index
                
                avg_data[exp]['p-value O2 transition full ' + exp] = p_val

    if 'AN' in exp:
        xaxis    = xaxis_AN
        exp_data = avg_data[exp]
        
        for i, phase in enumerate(xaxis[:3]):
            ...
            p_val = st.ttest_ind_from_stats(exp_data[mean1], exp_data[sd1], exp_data[n1], exp_data[mean2], exp_data[sd2], exp_data[n2], equal_var=False, alternative='two-sided')[1]
            p_val = pd.DataFrame(p_val)
            p_val.columns = ['p-value AN transition '+exp+'_'+xaxis[i+1]]
            p_val.index = exp_data.index
            
            avg_data[exp]['p-value AN transition '+exp+'_'+xaxis[i+1]] = p_val
            ...
            if phase == 'ME':
                ...
                p_val = st.ttest_ind_from_stats(exp_data[mean1], exp_data[sd1], exp_data[n1], exp_data[mean2], exp_data[sd2], exp_data[n2], equal_var=False, alternative='two-sided')[1]
                p_val = pd.DataFrame(p_val)
                p_val.columns = ['p-value AN transition full '+exp ]
                p_val.index = exp_data.index
                
                avg_data[exp]['p-value AN transition full '+exp] = p_val

#%% 10. FC plots glycolysis with significance score MG vs WT
...
#%% 10. FC plots glycolysis with significance score O2 vs AN

print("\n\033[4m10b. Make fold change plots of the glycolytic proteins to compare the WT AN and O2 data\033[0m")
...
fig, axs            = plt.subplots(math.ceil(len(gene_list)/4),4, figsize = (7.5*4, 6*math.ceil(len(gene_list)/4)))
...
for k, gene in enumerate(gene_list):
    ...
    for exp in avg_data:
        ...
        #only compare the WT data, so skip MG
        if 'MG' in exp:
            continue
        ...
        if 'AN' in exp:
            xdata=xdata_AN
            xdata_sign = [7.5, 10.5, 13.5, 16.5]
            xdata_sign = [x - 13.5 for x in xdata_sign]
        
        elif 'O2' in exp:
            xdata=xdata_O2
            xdata_sign = [6, 9, 12, 27]
            xdata_sign = [x - 12 for x in xdata_sign]
        ...
        if gene in list(avg_data[exp]['Gene name']):
            ...
            if out_plot.isnull().sum().sum() < 3:
                ...
                label = exp
                lns = axs[i, j].errorbar(xdata, list(out_plot.iloc[0]), fmt = fmt, yerr = yerr, fillstyle = fillstyle[color_i], color = color[color_i], linewidth = 3, markersize = 7, label=exp+ ' ' + str(unique), capsize = 5, elinewidth = 1.2, alpha=0.7)
                leg.append(lns)
                
                # only use the datapoints that represent the same phases between AN and O2
                xaxis = ['ME', 'LE', 'ES', 'MS']
                xdata = xdata_sign
                ...
                for l, phase in enumerate(xdata):
                    ...
                    if float(avg_data[exp][[col for col in avg_data[exp].columns if 'ANvsO2' in col 
                                            and xaxis[l] in col]].loc[gene]) < 0.001:
                        axs[i, j].annotate('***', xy = (xdata[l], annot), xytext = (xdata[l]+0.3, annot), 
                                           color = 'k', fontsize = 12)
                    elif float(avg_data[exp][[col for col in avg_data[exp].columns if 'ANvsO2' in col 
                                              and xaxis[l] in col]].loc[gene]) < 0.01:
                        axs[i, j].annotate('**', xy = (xdata[l], annot), xytext = (xdata[l]+0.3, annot), 
                                           color = 'k', fontsize = 12)
                    elif float(avg_data[exp][[col for col in avg_data[exp].columns if 'ANvsO2' in col 
...
fig.tight_layout(w_pad=2)
folder  = folder_FC
figname = 'FC_plots_glycolysis_significance_O2_WT.pdf'
fig.savefig(folder + '/' + figname, bbox_inches = 'tight')

#%% 11. STRING enrichment and volcano plot compare WT to MG
...
#%% 11. STRING enrichment and volcano plot compare AN to O2

print("\n\033[4m11b. STRING functional enrichment analysis comparing the WT strain under aerobic vs anaerobic conditions\033[0m")
...
#concatenate the AN and O2 data of the wildtype experiments for normalisation
exp_data_WT = pd.DataFrame()
...
#select phases you want to compare
xaxis     = ['ME', 'LE', 'ES', 'MS']
...
#normalize each phase to the O2 data
for phase in xaxis:
    if phase == 'ES':
        col_ratio = [col for col in keep_cols if phase in col or 'ED' in col]
        new_ratios.loc[:,col_ratio] = new_ratios[col_ratio].div(exp_data[[col for col in keep_cols if 'ED' in col and 'O2' in col][0]], axis=0)
    
    else:
        col_ratio = [col for col in keep_cols if phase in col]
        new_ratios.loc[:,col_ratio] = new_ratios[col_ratio].div(exp_data[[col for col in keep_cols if phase in col and 'O2' in col][0]], axis=0)
...
# determine sign up- and downregulated genes for each phase, and make volcano plots
for phase in xaxis:
    lfc = [col for col in exp_data.columns if 'Log2 FC' in col and 'AN' in col and phase in col and 'full' not in col][0]
    pv  = [col for col in exp_data.columns if 'p-value' in col and 'ANvsO2' in col and phase in col][0]
    equal, upregulated, downregulated = apf.up_down_regulated(exp_data, lfc = lfc, pv=pv, lfc_thr=lfc_thr)
    upregulated_g     =   upregulated['Gene name'].tolist()
    downregulated_g   = downregulated['Gene name'].tolist()
    upregulated     =   upregulated['Accession'].tolist()
    ...
    # make and save volcano plot
    figname  = 'Volcano_plot_ANvsO2_'+ phase+'.pdf'
    figname  = folder_volcano+'/'+figname
    figtitle = phase+' phase'    
    df = apf.gene_exp.volcano(df=exp_data2, lfc = lfc, pv = pv, lfc_thr=lfc_thr, figname=figname, title = figtitle, show=True, xlm=[-4,2, 1], ylm=[0,7.5,1]) 

exp                 = 'WT'        
all_Changes[exp]    = Changes
all_Changes_g[exp]  = Changes_g
filename            = folder + '/STRING_ANvsO2_' + exp
...
#%% 11. STRING enrichment and volcano plot (an)aerobic transition into stationary

print("\n\033[4m11c. STRING functional enrichment analysis comparing the (an)aerobic transition into stationary phase of the WT strain\033[0m")
...
#specify which phases you want to compare: either xaxis_AN, xaxis_O2 or only ['ME', 'MS']
xaxis     = ['ME', 'MS']

#specify which experiment you are looking at: ether 'WT_AN' or 'WT_O2'
exp       = 'WT_AN'
exp_data  = avg_data[exp]

#retrieve a set of background genes to give as input to the STRING enrichment tool
string_background     = exp_data.copy()
string_background     = string_background.dropna(subset=[col for col in exp_data.columns if 'Unique' in col], how = 'all')
all_string_identifier = apf.backgroundgene_2_string(list(string_background.index))
...
#normalize each phase to the previous phase
for i, phase in enumerate(xaxis[:(len(xaxis)-1)]):
    col_ratio  = [col for col in keep_cols if xaxis[i] in col]
    col        = [col for col in keep_cols if xaxis[i+1] in col]
    temp_ratio = exp_data[col].div(exp_data[col_ratio[0]], axis=0) 

    #save recalculated ratios and the log2 of the ratios to the dataframe 
    log2FC     = np.log2(temp_ratio.astype(float))
    new_cols   = []
    for col in log2FC.columns:
        new_cols.append(col.replace('Ratio', 'Log2 FC'))
    if xaxis == ['ME', 'MS']:
        new_cols = [col+' full' for col in new_cols]
    exp_data[new_cols] = log2FC
    
    # determine sign up- and downregulated genes for each phase, and make volcano plots
    lfc = [col for col in exp_data.columns if 'Log2 FC' in col and xaxis[i+1] in col][0]
    pv  = [col for col in exp_data.columns if 'p-value' in col and 'transition' in col and xaxis[i+1] in col][0]
    
    if xaxis == ['ME', 'MS']:
        lfc = [col for col in exp_data.columns if 'Log2 FC' in col and xaxis[i+1] in col and 'full' in col][0]
        pv  = [col for col in exp_data.columns if 'p-value' in col and 'transition' in col and 'full' in col][0]
    ...
    upregulated           =   upregulated['Accession'].tolist()
    downregulated         = downregulated['Accession'].tolist()
    Changes[xaxis[i+1]]   = {'upregulated': upregulated, 'downregulated': downregulated}
    Changes_g[xaxis[i+1]] = {'upregulated': upregulated_g, 'downregulated': downregulated_g}
    ...
    # make and save volcano plot
    figname = 'Volcano_plot_' + exp +'_'+ xaxis[i+1]+'.pdf'
    figname = folder_volcano + '/' + figname
    figtitle = xaxis[i+1] + ' phase vs ' + xaxis[i] + ' phase'
    apf.gene_exp.volcano(df=exp_data2, lfc = lfc, pv = pv, lfc_thr=lfc_thr, figname=figname, title = figtitle, show=True, dotsize = 10)#, geneid = ['Gene name'], genenames='deg', gstyle=1, gfont=9) 
    
    

all_Changes[exp]    = Changes
filename            = folder + '/string_transition_' + exp

print1              = exp
print2              = 'previous phase'
...
"""
pathways of interest shown in the thesis:
    - glycogen metabolism
    - glycerol metabolism
    - heat shock proteins
    - mitochondrial proteins from Binai (2015)
    - ROS related proteins Sod1, Sod2, and Cta1
    - beta-oxidation proteins Faa2 and Pox1
...
#gemove genes from the genelist that were not found in any of the biological replicates
genelist = [gene for gene in genelist if gene in list(avg_data['MG_AN'].index)]
genelist = sorted(genelist) #sort alphabetically

Out_gene_combined = {}

fig, axs = plt.subplots(1,len(genelist), figsize = (7*len(genelist), 6*math.ceil(len(genelist)/len(genelist))))
...
for k, gene in enumerate(genelist):
    ...
    for exp in avg_data:
        ...
        #comment the two lines below if you want to include the MG data
        if 'MG' in exp:
            continue
        ...
        if gene in list(avg_data[exp]['Gene name']):
            ...
            if out_plot.isnull().sum().sum() < 3:
                ...
                label  = exp
                lns = axs[j].errorbar(xdata, (list(out_plot.iloc[0])), fmt = fmt, yerr = yerr, 
                                      fillstyle = fillstyle[color_i], color = color[color_i], 
                                      linewidth = 3, markersize = 9, label=exp+ ' ' + str(unique), 
                                      capsize = 5, elinewidth = 1.2, alpha=0.7, zorder=2)
                leg.append(lns)
                ...
                for l, phase in enumerate(xdata):
                    ...
                    #which differences do you want to significantly compare?
                    compare = 'ANvsO2'         #choose between 'MGvsWT' or 'ANvsO2'
                    ...
                    if float(avg_data[exp][[col for col in avg_data[exp].columns if O2AN in col and 
                                            compare in col and xaxis[l] in col]].loc[gene]) < 0.001:
                        axs[j].annotate('***', xy = (xdata[l], annot), xytext = (xdata[l]+0.3, annot), 
                                        color = 'k', fontsize = 12)
                    elif float(avg_data[exp][[col for col in avg_data[exp].columns if O2AN in col and 
                                              compare in col and xaxis[l] in col]].loc[gene]) < 0.01:
                        axs[j].annotate('**', xy = (xdata[l], annot), xytext = (xdata[l]+0.3, annot), 
                                        color = 'k', fontsize = 12)
                    elif float(avg_data[exp][[col for col in avg_data[exp].columns if O2AN in col and 
                                              compare in col and xaxis[l] in col]].loc[gene]) < 0.05:
                        axs[j].annotate('*', xy = (xdata[l], annot), xytext = (xdata[l]+0.3, annot), 
                                        color = 'k', fontsize = 12)
                    elif float(avg_data[exp][[col for col in avg_data[exp].columns if O2AN in col and 
                                              compare in col and xaxis[l] in col]].loc[gene]) < 0.1:
                        axs[j].annotate('^', xy = (xdata[l], annot), xytext = (xdata[l]+0.3, annot), 
                                        color = 'k', fontsize = 8)
                        
                    if xaxis[l] == 'MS':
                        if float(avg_data[exp][[col for col in avg_data[exp].columns if O2AN in col and 
                                                'transition' in col and 'full' in col]].loc[gene]) < 0.001:
                            axs[j].annotate('***', xy = (xdata[l], annot), xytext = (xdata[l]+0.7, annot+0.04), 
                                            color = 'g', fontsize = 16, fontweight='bold')
                        elif float(avg_data[exp][[col for col in avg_data[exp].columns if O2AN in col and 
                                                  'transition' in col and 'full' in col]].loc[gene]) < 0.01:
                            axs[j].annotate('**', xy = (xdata[l], annot), xytext = (xdata[l]+0.7, annot+0.04), 
                                            color = 'g', fontsize = 16, fontweight='bold')
                        elif float(avg_data[exp][[col for col in avg_data[exp].columns if O2AN in col and 
                                                  'transition' in col and 'full' in col]].loc[gene]) < 0.05:
                            axs[j].annotate('*', xy = (xdata[l], annot), xytext = (xdata[l]+0.7, annot+0.04), 
                                            color = 'g', fontsize = 16, fontweight='bold')
                        elif float(avg_data[exp][[col for col in avg_data[exp].columns if O2AN in col and 
                                                  'transition' in col and 'full' in col]].loc[gene]) < 0.1:
                            axs[j].annotate('^', xy = (xdata[l], annot), xytext = (xdata[l]+0.7, annot+0.04), 
    ...
    #adjust characteristics of the subplot        
    axs[j].set_title(gene.title(), fontsize = 19, fontweight = 'bold')
    axs[j].legend(leg, labs, fontsize = 15, loc='best')
    axs[j].set_xticks([int(item) for item in xdata_AN]+xdata_O2)
    axs[j].set_xticklabels([int(item) for item in xdata_AN]+xdata_O2, fontsize = 15)
    axs[j].tick_params(axis='y', labelsize = 15)
    axs[j].set_xlabel('Time (h)', fontsize = 17)
    axs[j].set_ylabel('Fold change', fontsize = 17)
    axs[j].grid(True, linewidth=0.2, color='lightgrey', alpha=0.6)
    axs[j].set_xlim(right=17, left=-7)
    #axs[j].set_ylim(top=6.5, bottom=0)
...
fig.tight_layout(w_pad=2)
folder = folder_FC
figname = 'FC_plots_methionine.pdf'
#fig.savefig(folder + '/' + figname, bbox_inches = 'tight')